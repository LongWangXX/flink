 ...
 *
 *
 * StreamingGraph是执行计划生成的第一张图。它包含两个重要元素，StreamNode和StreamEdge。
 * StreamGraph的生成发生在用户调用了env.execute() 方法之后，而在这之前，
 * 用户编写的应用程序会转换成一个包含了Transformation的集合，关于具体的Transformation接下来就会介绍。
 * 接着根据这个Transformation集合来生成相应的StreamNode并且用StreamEdge连接起来形成一张图。
