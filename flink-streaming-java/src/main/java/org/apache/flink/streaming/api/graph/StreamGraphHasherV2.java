...
public class StreamGraphHasherV2 implements StreamGraphHasher {
	...
	private boolean generateNodeHash(
		...
		//获取user定义的uuid。算子调用user.setuid
		String userSpecifiedHash = node.getTransformationUID();
		//如果用户设置了uid。则调用用户的uid生成hash值。
		if (userSpecifiedHash == null) {
			...
			for (StreamEdge inEdge : node.getInEdges()) {
				...
				//如果输入节点没有被计算那么先返回false。等输入节点计算之后再计算当前的node
	...
	private byte[] generateDeterministicHash(
		...
		//在当前StreamNode之前已经处理过的节点的个数，作为当前StreamNode的 id，添加到hasher中；
		generateNodeLocalHash(hasher, hashes.size());

		// Include chained nodes to hash
		for (StreamEdge outEdge : node.getOutEdges()) {
			//判断变是否可以和出边对应的顶点组合能合成一个chan。
			//如果可组成chan那么则再次使用zize传入hash值生成器中。
