 ...
 *
 * API operations such as DataStream#map create
 ...
 *
 * A {@code Transformation} does not necessarily correspond to a physical operation
 * at runtime. Some operations are only logical concepts. Examples of this are union,
 * split/select data stream, partitioning.
 *
 * The following graph of {@code Transformations}:
 * {@code
 *   Source              Source
 *      +                   +
 *      |                   |
 ...
 *  Transformation 每一个转换算子每一次操作转换DataStream都会生成一个Transformation。
 *  Transformaion包含以下几类
 *    算子类
 *    partitioner类
 *    连接类
